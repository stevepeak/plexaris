...
## Phase 1: Move org data to tRPC (eliminates duplicate `/api/organizations/mine`)
...
## Phase 2: Optimize middleware org check (eliminates middleware `/api/organizations/mine` call)
...
## Phase 3: Accept single client session call (no code change)
